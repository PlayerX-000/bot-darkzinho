...
let atacante = []
let defensor = []
if(def_user[0]===undefined) return await conn.sendMessage(id, `Desafiante não encontrado`,MessageType.text)

defensor.nome = await conn.contacts[numeroDefensor+"@s.whatsapp.net"]
atacante.nome = await conn.contacts[num_cll+"@s.whatsapp.net"]

 defensor.nome =  defensor.nome.notify
 atacante.nome =  atacante.nome.notify
// DADOS USUARIO QUE ATACA
const xp1 = atk_user[0].dataValues.xp 
const nomeatk = atacante.nome
...
if(atk1<1)return  await conn.sendMessage(id, `
...
if(xp1<=0)return  await conn.sendMessage(id, `
Você não tem experiencia(XP) suficiente.
XP atual: ${xp1}.
Aumente seu XP conversando e apostando.
`,MessageType.text)
