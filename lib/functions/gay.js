...
nome_s.push(users[escolhido])
   ...
   🌈⎨⊶.⊷⊷⊷⊷(${porc}%)⊶⊷⊷⊷
    `
         ...
         const url=`./public/img/gay/${escolhido_i}.jpeg`
        const buffer = await readFile(url)
         
    await conn.sendMessage(id, { url: url},MessageType.image, { mimetype: Mimetype.jpeg, caption: txt_gay, contextInfo: { "mentionedJid": [nome_s[0].jid] } })
    }
